...
def fog(x, severity=1):
    ...
    x = np.array(x) / 255.
    max_val = x.max()
    x += c[0] * plasma_fractal(mapsize=next_power_of_2(max(x.shape[0],x.shape[1])),wibbledecay=c[1])[:x.shape[0], :x.shape[1]][..., np.newaxis]
...
def frost(x, severity=1):
    ...
    filename = [resource_filename(__name__, 'frost/frost2.png'),
    ...
    if frost.shape[0]>=h and frost.shape[1]>=w:
